...
class AccountController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $accounts = Account::query()
            ->when($type === 'staff',
                fn ($q) => $q->whereIn('role', ['admin', 'employee'])->with('employee'),
    ...
    * Job title and specialty stay with EmployeeController, and active status with
     * toggle(), so each of those rules lives in exactly one place.
     */
    public function edit(Account $account)
    {
        $account->load(['employee', 'patient']);
